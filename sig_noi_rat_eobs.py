...
def CombineEarlyLate(early,late,eobslat,eobslon):
    """
    """
    varfull = pl.zeros([early.shape[0]+late.shape[0],eobslat.size,eobslon.size])
...
ncx = xr.open_dataset(eobsdir+VAR+'_seasmean_0.25deg_reg_v23.0e.nc')
eobslat = xr.DataArray(ncx.latitude)
eobslon = xr.DataArray(ncx.longitude)
eobstime = xr.DataArray(ncx.time)
eobsdata = xr.DataArray(getattr(ncx,VAR))
#t2m = xr.DataArray(ncx.t2m)
#tp = xr.DataArray(ncx.tp)
#swvl1 = xr.DataArray(ncx.swvl1)
ncx.close()

#eobsdata.data[0,:,:] = pl.float64('nan')
#eobsdata.data[-1,:,:] = pl.float64('nan')

eobsdata_jja = eobsdata[1:-26:4,:,:]
eobsdata_djf = eobsdata[3:-26:4,:,:]
...
nc_gmst = xr.open_dataset(eobsdir+'tg_seasmean_remapbil1.0_v23.0e_gm.nc')
gmst = xr.DataArray(nc_gmst.tg)
nc_gmst.close()

#gmst.data[0] = pl.float64('nan')
#gmst.data[-1] = pl.float64('nan')

gmst_jja = gmst.data[1:-1:4]
gmst_djf = gmst.data[3:-1:4]
...
SN_jja, SN_djf = S_N_ratio(eobsdata_jja,eobsdata_djf,eobslat,eobslon,gmst_smth_all)
...
cs = ax1.contourf(eobslon.data,eobslat.data,SN_jja,transform=proj,cmap='seismic_r',
                 levels=pl.linspace(-2,2,9),extend='both',alpha=0.8)
cn = ax1.contour(eobslon.data,eobslat.data,SN_jja,transform=proj,
...
ax1.annotate('JJA eobs '+VAR+' S/N',(-13.8,69.3),bbox={'facecolor':'w'},
...
cs = ax2.contourf(eobslon.data,eobslat.data,SN_djf,transform=proj,cmap='seismic_r',
                 levels=pl.linspace(-2,2,9),extend='both',alpha=0.8)
cn = ax2.contour(eobslon.data,eobslat.data,SN_djf,transform=proj,
                 levels=[-1,0,1,2,3],colors='k',linestyles='solid')
ax2.clabel(cn,manual=True,fmt="%0.0f")#pl.colorbar(cs,orientation='horizontal')
GridLines(ax2,False,True,False,True)
ax2.annotate('DJF eobs '+VAR+' S/N',(-13.8,69.3),bbox={'facecolor':'w'},
             xycoords='data',zorder=50)
...
pl.savefig(indecis+'figures/S_N_ratio_eobs_'+VAR+'.png',dpi=375)